...
df <- mutate(df, id = paste0(lob, GRCODE))
  
id_missing <- unique(df$id[df[, "CumPaidLoss_D"] <= small_cut])
...
mack_f <- function(triangles) {
  # run mack model as provided in ChainLadder package
  mack <- lapply(triangles, MackChainLadder, est.sigma = "Mack")
  
  # extract the good stuff from the mack projection
  mack_smry <- vector("list", length(mack))
  for (i in seq_along(mack)) {
    mack_smry[[i]] <- summary(mack[[i]])[[2]]$Totals[c(1, 3:5)]
  }

  mack_smry <- as.data.frame(t(as.data.frame(mack_smry)))
  rownames(mack_smry) <- NULL
  mack_smry <- data.frame(names(triangles), mack_smry)
  names(mack_smry) <- c("id", "latest", "mack_ultimate", "mack_ibnr", "mack_se")
  mack_smry
}

paid_mack_smry <- mack_f(paid_triangles)
incurred_mack_smry <- mack_f(incurred_triangles)
```
...
paid_actual_ultimate <- actual_ultimate(data_sets, loss_type = "CumPaidLoss_D")
incurred_actual_ultimate <- actual_ulptimate(data_sets, loss_type = "IncurredLoss_D")
...
paid_mack_full <- mack_and_actual(paid_mack_smry, 
                                         paid_actual_ultimate)


# combine incurred mack projections with actuals
incurred_mack_full <- mack_and_actual(incurred_mack_smry, 
                                         incurred_actual_ultimate)
```

```{r mack_histogram, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(paid_mack_full, aes(x = z)) +
       geom_histogram(fill = "white", colour = "black") +
       xlab("Standard Normal") +
       ggtitle("MackChainLadder - Paid Projection")

ggplot(incurred_mack_full, aes(x = z)) +
       geom_histogram(fill = "white", colour = "black") +
       xlab("Standard Normal") + 
