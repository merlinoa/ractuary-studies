...
```{r mack_projections, warning = FALSE}
# function for mack
...
combine_mack_actual <- function(mack, actual) {
  mack_smry <- left_join(mack, actual, by = "GRCODE")
  names(mack_smry)[length(mack_smry)] <- "actual_ultimate"
  mutate(mack_smry, actual_ibnr = actual_ultimate - latest)
}

#full_mack_smry <- vector("list", length(paid_mack_smry))
#for (i in seq_along(paid_mack_smry)) {
#  full_mack_smry[[i]] <- 
#}
#paid_mack_smry <- mutate(paid_mack_smry, paid_actual_ibnr = actual_ultimate - latest)
mack_smry
```
