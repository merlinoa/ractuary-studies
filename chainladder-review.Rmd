...
A good portion of the companies in the data sets are missing losses for a portion of the origin years (e.g. for workers compensation 70 out of the 132 companies have a cumulative paid loss of zero at at least one origin year and development period).  In addition to companies with incomplete loss information we removed several companies with highly irregular loss development. To see all the code to create this report see the [.Rmd file on github](https://raw.githubusercontent.com/merlinoa/ractuary-studies/master/chainladder-review.Rmd).
...
The analysis is done in R and, as far as I can tell, should be 100% reproducible assuming you have the appropriate free software installed on your computer.  The analysis takes each of the companies in the 6 lines of businesss represented in the CAS data sets, and runs several `ChainLadder` reserve projection functions.  I then took the standard normal of the projections in relation to actual losses and plotted the standard normals in histograms.  There are two histograms, one for the paid projection and one for the incurred projection, for each `ChainLadder` function covered in this document.  
...
mack_f <- function(triangles) {
  ...
  mack_smry$lob <- gsub("_.*$", "", mack_smry$id)
  mack_smry
}
...
```{r mack_histogram, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4}
...
```{r boot_projections, echo = FALSE, warning = FALSE}
# function for boot=================================================================
# param triangles list of all company triangles
...
paid_boot_smry <- projection(paid_triangles, method = "BootChainLadder")
head(paid_boot_smry)
```
...
We are interested in the `Totals` row `mack_ibnr` and `mack_se` from the table above for each company.  A summary of the totals for a few of the workers' compensation companies looks as follows.  Notice that the company we looked at in the above tables is in row 2 of the following table.  We compare the Mack projections to the actual ultimate to get the `z` column in the table below.  The `z` column is the value plotted in the histograms.
