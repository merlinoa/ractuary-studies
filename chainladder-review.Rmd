...
```{r z_function}
# param clean_data the cleaned insurance loss data frame
# param lob numeric the desired line of business location
# param loss_type character "paid" or "incurred"
triangles <- function(clean_data, lob, loss_type) {
 # isolate data for individual line of business
 lb <- data_sets[[lob]]

 # find unique GRCODEs for looping
 lb_id <- unique(lb$GRCODE)
 ...
 tri <- vector("list", length(lb_id))
 for (i in seq_along(lb_id)) {
   tri[[i]] <- as.triangle(lb[lb$GRCODE == lb_id[i] & 
                                      lb$AccidentYear + 
                                      lb$DevelopmentLag < 1999, ], 
                               origin = "AccidentYear", 
                               dev = "DevelopmentLag", 
                               value = loss_col_name)
 }
...
# define function for paid lapply
paid_tris <- function(x) {
  triangles(clean_data = data_sets, 
                         lob = x,
                         loss_type = "paid")
}

# define function for incurred lapply
incurred_tris <- function(x) {
  triangles(clean_data = data_sets, 
                         lob = x,
                         loss_type = "incurred")
}
...
actual_ultimate <- function(clean_data) {
  ...
  filter(clean_data, DevelopmentLag == 10) %>%
    group_by(GRCODE) %>%
    summarise(actual_ultimate = loss_col_name)
}
