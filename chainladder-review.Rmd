...
The `ChainLadder` package by Markus Gesmann, Dan Murphy, and Wayne Zhang includes methods for projecting insurance reserves.  An insurance reserve is the liability an insurance policy has to pay for claims.  The correct value of the reserve, at any given time, is equal the ultimate loss amount of the insurance claims less the amount that has already been paid.  Accurately projecting reserves is a major area of focus for actuaries, and many methods have been developed for projecting reserves.  

The Mack and Boot reserve projection methods are calculated by the `ChainLadder` functions, `MackChainLadder()` and `BootChainLadder()`.  `MackChainLadder()` provides a distribution free calculation of the standard error for the chain ladder reserve projection.  `BootChainLadder()` uses bootstap resampling to determine the standard error of the reserve projection.  For more information please see the `ChainLadder` [package vignette](http://cran.r-project.org/web/packages/ChainLadder/vignettes/ChainLadder.pdf).

### Data

The CAS website provides historic U.S. insurance loss data across 6 different lines of business and many different companies.  Data from origin years 1988 through 1997 evaluated as of 1997 (i.e. the upper left triangle of losses) is used to run the `ChainLadder` projections.  The CAS data sets include actual losses developed for 10 years for all the provided origin years.  The following lines of business are included in the data set:
...
The analysis runs the `MackChainLadder` and `BootChainLadder` reserve projection functions on each company.  I then identify what the true reserve should have been and subtract the projected reserve amount from the true reserve to find the error in the projection.  To compare the error in the projection across the different companies with varying exposure amounts I divide the projection error by the standard error of the projection estimated by the `ChainLadder` reserving function.  The analysis is simply interested in the value $Z$, where:

$Z = \frac{(p - \hat{p})}{\hat{se(\hat{p})}}$

Where $\hat{p}$ is the projected reserve $\hat{se(\hat{p})}$ is the estimated standard error of the projected reserve and $p$ is the actual reserve.
...
```{r histogram_plots, echo = FALSE, message = FALSE}
...
The `BootChainLadder()` function on incurred losses gave significantly more accurate reserve projections than what would be expected if the projection was normally distributed around the mean.