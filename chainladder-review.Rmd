...
I run the `MackChainLadder` and `BootChainLadder` reserve projection functions on each company.  I subtract the projected reserve from the actual reserve (paid after 10 development year less paid at time of projection) to arrive at the projection error.  To compare the projection error across the different companies, I divide the projection error by the estimated projection standard error.  Put a little more formally, I are simply interested in the value $Z$, where:
...
* $p$ is the actual reserve.  Where the actual reserve is the paid amount after 10 development years less the paid amount at the time of the projection estimate.
...
Both the Mack and Boot paid reserve projections appear to slightly underestimate the actual reserve across all lines of business.  Several factors including inflation differences could be responsible for this underestimation.
...
The `BootChainLadder()` function on incurred losses gave significantly more accurate reserve projections than what would be expected if the projection errors were normally distributed around the projection.
