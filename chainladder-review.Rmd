...
A good portion of the companies in the data sets are missing loss information for a portion of the origin years (e.g. for workers compensation 70 out of the 132 companies do not have loss information for all origin years at all development periods).  In addition to companies with missing loss information we removed companies with any cumulative paid loss amounts less than `r small_cut`. To see all the code to create this report see the [.Rmd file on github](https://raw.githubusercontent.com/merlinoa/ractuary-studies/master/chainladder-review.Rmd).
...
data_sets <- rbind(wkcomp, ppauto, prodliab, comauto, medmal, othliab)
data_sets$lob <- as.factor(data_sets$lob)
...
data_sets <- mutate(data_sets, id = paste0(lob, GRCODE))
  
id_missing_data <- unique(data_sets$id[data_sets[, "CumPaidLoss_D"] <= small_cut])

# remove companies that have missing data
data_sets <- data_sets[-which(data_sets$id %in% id_missing_data), ]
```

# Analysis

The analysis is done in R and, as far as I can tell, should be 100% reproducible assuming you have the appropriate free software installed on your computer.  The analysis takes each of the companies in the 6 lines of businesss represented in the CAS data sets, and runs several `ChainLadder` reserve projection functions.  I then standard normalize the projections in relation to actual losses and plot the standard normals in histograms.  There are two histogram, one for the paid projection and one for the incurred projection, for each `ChainLadder` function covered in this document.  

Several steps are invloved in creating the desired visualizations from the cleaned data.  An example of each of these steps is outlined in \hyperref[appendix:a]{Appendix A} for the Mack Method on paid workers' compensation losses. 

## The Plots
```{r create_triangles, echo = FALSE, eval = FALSE}
...
```{r mack_projections, echo = FALSE, warning = FALSE, eval = FALSE}
# function for mack=================================================================
...
```{r compare_mack_actual, echo = FALSE, eval = FALSE}
...
actual_ultimate <- function(lob, loss_type) {
  ...
  dev_10_sum <- as.data.frame(tapply(dev_10[, loss_col_name], dev_10$GRCODE,  sum))
  dev_10_sum$GRCODE <- rownames(dev_10_sum)
...
mack_and_actual <- function(mack, actual) {
  ...
  mutate(mack_smry, 
         ...
         z = (actual_ibnr - mack_ibnr) / mack_se)
}

# combine mack projections with actual
...
# combine mack projections with actuals
incurred_mack_full <- vector("list", length(incurred_mack_smry))
for (i in seq_along(incurred_mack_smry)) {
  incurred_mack_full[[i]] <- mack_and_actual(incurred_mack_smry[[i]], 
                                         incurred_actual_ultimate[[i]])
}

```

```{r mack_histogram, echo = FALSE, warning = FALSE, message = FALSE, eval = FALSE}
ggplot(paid_mack_full[[1]], aes(x = z)) +
       geom_histogram(fill = "white", colour = "black") +
       xlab("Standard Normal") +
       ggtitle("MackChainLadder - Paid Projection")

ggplot(incurred_mack_full[[1]], aes(x = z)) +
       geom_histogram(fill = "white", colour = "black") +
       xlab("Standard Normal") + 
...
wkcomp_clean <- data_sets[data_sets$lob == "wkcomp", ]
...
We can apply the `ChainLadder` function `MackChainLadder()` to each of our triangles.  The following table is a summary of the `MackChainLadder()` function applied to the triangle above.
...
We are only interested in the `Totals` row for each company.  A summary of the totals for a few of the workers' compensation companies looks as follows.  Notice that the company we looked at in the above tables is in row 2 of the following table.
...
mack_smry_f <- xtable(mack_smry, 
                      digits = c(0, 0, 0, 0, 0, 0, 0, 2),
                      align = "ccrrrrrr",
                      display = c("d", "s", "d", "d", "d", "d", "d", "d"),
                      format.args = list(big.mark = ","))
...
out_one_se <- abs(mack_smry$mack_ibnr - mack_smry$actual_ibnr) > 
                  mack_smry$mack_se
out_two_se <- abs(mack_smry$mack_ibnr - mack_smry$actual_ibnr) > 
                 (mack_smry$mack_se * 2)
...
A quick review of the projections shows us that only `r round(one_se * 100, 1)`% of the actual insurance loss amounts were within one Mack S.E. of the projected ultimate loss amounts, and  `r round(two_se * 100, 1)`% were within two Mack S.E.